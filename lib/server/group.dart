...
class Group {
  ...
  /// create a group
  static Future<GroupResponse> create(CreateUpdateGroupRequest requestData) async {
  ...
  /// update a group
  static Future<void> update(String groupId, CreateUpdateGroupRequest requestData) async {
    var jsonBody = json.encode(requestData.toJson());

    http.Response response = http.Response("", 500);
    try {
      response = await _apiServer.post(sprintf(groupUrl, [groupId]), jsonBody);
    } catch (err) {
      print(err);
    }
  ...
  static Future<void> getMembers(String groupId, {bool withVotes= false}) async {
    ...
    // TODO should return List<UserResponse>
  }
